...
def sitemap_urls():
    if not XLSX.exists():
        print(f"Warning: {XLSX} not found. Skipping sitemap.")
        return []
...
def article_paths():
    paths = []
    if not ZIP.exists():
        print(f"Warning: {ZIP} not found. Skipping article paths.")
        return paths
    try:
...
def media_data():
    ...
    if not MEDIA_XML.exists():
        print(f"Warning: {MEDIA_XML} not found.")
    ...
    try:
        ...
        print(f"Error parsing media XML: {e}")
    return paths, records
